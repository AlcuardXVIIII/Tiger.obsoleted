...
Tr_exp Tr_simpleVar(Tr_access access,Tr_level level){
  F_access f_access = access->access;
  Tr_level tr_level = access->level;
  T_exp t_exp = staticLink(level,tr_level);

  return Tr_Ex(F_Exp(f_access,t_exp));
}
T_exp staticLink(Tr_level inLevel,Tr_level desLevel){
  T_exp t_exp = T_Temp(F_FP());
  while(inLevel!=desLevel->parent){
    t_exp = T_Mem(T_Binop(T_plus,T_Const(F_wordSize),t_exp));
    inLevel = inLevel->parent;
  }
  return t_exp;
}
