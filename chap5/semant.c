...
Tr_expty transVar(S_table venv,S_table tenv,A_var v){
  ...
  case A_fieldVar:{
    Tr_expty upLevel = transVar(venv,tenv,v->u.field.var);
  ...
  case A_subscriptVar:{
    Tr_expty var_expty = transVar(venv,tenv,v->u.subscript.var);
    Tr_expty exp_expty = transExp(venv,tenv,v->u.subscript.exp);
...
Tr_expty transExp(S_table venv,S_table tenv,A_exp a){
  switch(a->kind){
  case A_opExp:{
    A_oper oper = a->u.op.oper;

    Tr_expty left = transExp(venv,tenv,a->u.op.left);

    Tr_expty right = transExp(venv,tenv,a->u.op.right);
  ...
  case A_callExp:{
    ...
    while(args!=NULL&& ty_tyList!=NULL){
      A_exp exp = args->head;
      Tr_expty  tr_expty = transExp(venv,tenv,exp);
  ...
  case A_letExp:{
    ...
    //+++++
    exp = transExp(venv,tenv,A_SeqExp(a->pos,a->u.let.body));
    S_endScope(venv);
    S_endScope(tenv);
    return exp;
  }
  ...
  case A_recordExp:{
    ...
    if(actual_ty!=NULL){
      ...
      while(a_efields!=NULL&&ty_fieldList!=NULL){
        ...
        Ty_ty ty_ty = transExp(venv,tenv,a_efield->exp)->ty;
  ...
  case A_seqExp:{
    ...
    while(explist!=NULL){
      ...
      tr_expty = transExp(venv,tenv,exp);
  ...
  case A_assignExp:{
    ...
    Tr_expty var_expty = transVar(venv,tenv,var);
    Tr_expty exp_expty = transExp(venv,tenv,exp);
  ...
  case A_ifExp:{
    ...
    Tr_expty test_expty = transExp(venv,tenv,test_exp);
    Tr_expty then_expty = transExp(venv,tenv,then_exp);
    if(test_expty->ty->kind!=Ty_int){
      EM_error(a->pos,"test must be bool");
    }
    if(else_exp!=NULL){
      Tr_expty else_expty = transExp(venv,tenv,else_exp);
  ...
  case A_whileExp:{
    ...
    Tr_expty test_expty = transExp(venv,tenv,test);
    if(test_expty->ty->kind!=Ty_int){
  ...
  case A_forExp:{
    ...
    Tr_expty lo_expty = transExp(venv,tenv,lo);
    Tr_expty hi_expty = transExp(venv,tenv,hi);
  ...
  case A_arrayExp:{
    ...
    Tr_expty size_expty = transExp(venv,tenv,size);
    Tr_expty init_expty = transExp(venv,tenv,init);
...
void transDec(S_table venv,S_table tenv, A_dec d){
  ...
  case A_varDec:{
    if(d->u.var.typ==NULL){
      Tr_expty tr_expty = transExp(venv,tenv,d->u.var.init);
      if(!tr_expty->ty){
        ...
        S_enter(venv,d->u.var.var,E_VarEntry(tr_expty->ty));
      }
    }
    else{
      Ty_ty decType = actualTy(S_look(tenv,d->u.var.typ));
      Ty_ty initType = transExp(venv,tenv,d->u.var.init)->ty;
      if(decType == NULL){
        ...
        S_enter(venv,d->u.var.var,E_VarEntry(decType));
      }
    }
    break;
  }
  case A_functionDec:{
    ...
    while(fundecList!=NULL){
      ...
      S_enter(venv,fundec->name,E_FunEntry(ty_tyList,result_ty));
    ...
    while(fundecList!=NULL){
      ...
      while(ty_tyList!=NULL&&a_fieldList!=NULL){
        S_enter(venv,a_fieldList->head->name,E_VarEntry(ty_tyList->head));
        ty_tyList = ty_tyList->tail;
        a_fieldList = a_fieldList->tail;
      }
      Tr_expty tr_expty = transExp(venv,tenv,fundec->body);
      S_endScope(venv);
